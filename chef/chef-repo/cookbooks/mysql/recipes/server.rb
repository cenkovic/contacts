...
bash "create db" do
  code <<-EOH
    mysql -u root --password="#{node['mysql']['server_root_password']}" -e "CREATE DATABASE IF NOT EXISTS #{node['mysql']['dbname']}"  
    touch node['mysql']['dbflag']
    EOH
end

execute "import" do
  command "mysql -u root -p\"#{node['mysql']['server_root_password']}\" #{node['mysql']['dbname']} --default-character-set=utf8 < #{node['mysql']['default_sql_file']}"
  action :run
end
